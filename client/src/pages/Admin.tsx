  ...
  if (user?.role !== "admin") {
    return (
      <div className="min-h-screen bg-[#1D1D1F] flex items-center justify-center">
        <Navbar />
        <p className="text-white/50 text-lg">Acesso restrito a administradores.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#1D1D1F]">
      ...
      <section className="pt-32 pb-24">
        <div className="container">
          ...
          >
            <h1 className="text-headline text-white mb-8">Painel Administrativo</h1>
                  ...
                  <div key={product.id} className="bento-card !p-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white/40">{product.type} | R$ {parseFloat(product.price).toFixed(2)}</p>
                      </div>
                    </div>
                    <button
              ...
              <div className="space-y-3">
                {orders.map((order) => (
                  <div key={order.id} className="bento-card !p-4 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-white">Pedido #{order.id}</p>
                      <p className="text-xs text-white/40">
                        {new Date(order.createdAt).toLocaleDateString("pt-BR")} | {order.customerEmail || "N/A"}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold text-white">R$ {parseFloat(order.totalAmount).toFixed(2)}</p>
                ...
                {orders.length === 0 && <p className="text-center text-white/40 py-10">Nenhum pedido encontrado.</p>}
              </div>
            )}
              ...
              <div className="space-y-3">
                {leads.map((lead) => (
                  <div key={lead.id} className="bento-card !p-4">
                    ...
                    <p className="text-xs text-white/50">{lead.contactName} | {lead.email}</p>
                    {lead.message && <p className="text-xs text-white/30 mt-2">{lead.message}</p>}
                  </div>
                ))}
                {leads.length === 0 && <p className="text-center text-white/40 py-10">Nenhum lead encontrado.</p>}
              </div>
            )}
                  ...
                  <div key={u.id} className="bento-card !p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white/40">{u.email || "Sem email"}</p>
                      </div>
                    </div>
                    <span className={`text-xs px-2 py-1 rounded-full ${
